...
# Step 4: Split the columns into groups of 10 and save histograms for each group
num_columns = len(data.columns)
columns_per_group = 10

# Generate histograms for each group of 10 columns
for i in range(0, num_columns, columns_per_group):
    plt.figure(figsize=(15, 12))
    for j, col in enumerate(data.columns[i:i + columns_per_group]):
        plt.subplot(5, 2, j + 1)  # 5 rows, 2 columns grid for histograms
        data[col].hist(bins=15)
        plt.title(f"Histogram of {col}")
    
    # Save the plot to a file
    file_name = os.path.join(output_path, f"human_histogram{i // columns_per_group + 1}.png")
...
for feature in outlier_columns:
    ...
    plt.legend()
    # plt.show()
    plot_filename = f"human_scatter_{feature}.png"  # Use feature name in the file
...
sns.pairplot(data, hue='tBodyAcc-mean()-X', plot_kws={'alpha': 0.5})
plt.suptitle("Pairplot of Features with tBodyAcc-mean()-X Class Labels (Including Outliers)", y=1.02)
# plt.show()
plt.savefig(os.path.join(output_path, "human_outliers_body.png"))
...
sns.pairplot(cleaned_data, hue='tBodyAcc-mean()-X', plot_kws={'alpha': 0.5})
plt.suptitle("Pairplot of Features with tBodyAcc-mean()-X Class Labels (After Removing Outliers)", y=1.02)
# plt.show()
plt.savefig(os.path.join(output_path, "human_removed_outliers_body.png"))
...
data['tBodyAcc-mean()-X'] = label_encoder.fit_transform(data['tBodyAcc-mean()-X'])
...
for column in data.select_dtypes(include=['object']).columns:
    if column != 'tBodyAcc-mean()-X':  # Avoid encoding the target variable
        label_encoder = LabelEncoder()
        data[column] = label_encoder.fit_transform(data[column])

# Step 10: Define features (X) and target (y)

X = data.drop(columns=['tBodyAcc-mean()-X'])  # Features
y = data['tBodyAcc-mean()-X']  # Target variable
...
# plt.show()
plt.savefig(os.path.join(output_path, "human_metrics.png"))
...
plt.tight_layout()
# plt.show()
plt.savefig(os.path.join(output_path, "human_metrix.png"))
